...
\begin{cor}\label{C:MVcircPsiisId}
 ...
 $(\overline{~\cdot~ })^{\text{MV}} \circ \pi^{\text{MV}} \simeq \text{id}_{\D^b_{\R c}(\kk_\R)}$ between the composition of $\Psi$ followed by the Mayer-Vietoris sheafification and the identity functor of $\D^b_{\R c}(\kk_\R)$.
\end{cor}
\begin{pf}
 Since both functors $(\overline{~\cdot~ })^{\text{MV}}$ and $ \pi^{\text{MV}} $ commutes with shifts and direct sums (propositions~\ref{P:PropertiesofMVSheafification} and \ref{P:PropertiesofPsi}), in view of the structure theorem~\ref{T:KSdecomposition}, it is enough to construct the equivalence on sheaves of the form $\kk_{I}$. Now proposition~\ref{P:PropertiesofMVSheafification}.4 (for $\varepsilon=0$) and Proposition~\ref{P:PsionIntervals} precisely give the result for an interval.
\end{pf}
